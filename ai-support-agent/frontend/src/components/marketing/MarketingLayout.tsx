import { Link, Outlet, useLocation } from 'react-router-dom';
import { Menu, MessageSquare, X } from 'lucide-react';
import { useState } from 'react';

const navLinks = [
  { to: '/#features', label: 'Features' },
  { to: '/#how-it-works', label: 'How it works' },
  { to: '/#pricing', label: 'Pricing' },
];

const legalLinks = [
  { to: '/privacy', label: 'Privacy' },
  { to: '/terms', label: 'Terms' },
  { to: '/data', label: 'Data & consent' },
];

export default function MarketingLayout() {
  const [menuOpen, setMenuOpen] = useState(false);
  const location = useLocation();
  const isHome = location.pathname === '/';

  function navTo(path: string) {
    setMenuOpen(false);
    if (path.startsWith('/#') && isHome) {
      const id = path.slice(2);
      document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' });
      return;
    }
  }

  return (
    <div className="flex min-h-screen flex-col bg-paper text-ink-900">
      <header className="sticky top-0 z-40 border-b border-ink-200/60 bg-paper/90 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-5">
          <Link to="/" className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-ink-900">
              <MessageSquare className="h-4 w-4 text-white" />
            </div>
            <span className="text-sm font-semibold">SupportDesk</span>
          </Link>

          <nav className="hidden items-center gap-8 md:flex">
            {navLinks.map((link) =>
              link.to.startsWith('/#') ? (
                <a
                  key={link.to}
                  href={link.to}
                  onClick={(e) => {
                    if (isHome) {
                      e.preventDefault();
                      navTo(link.to);
                    }
                  }}
                  className="text-sm text-ink-600 hover:text-ink-900"
                >
                  {link.label}
                </a>
              ) : (
                <Link key={link.to} to={link.to} className="text-sm text-ink-600 hover:text-ink-900">
                  {link.label}
                </Link>
              )
            )}
          </nav>

          <div className="hidden items-center gap-3 md:flex">
            <Link to="/login" className="text-sm font-medium text-ink-600 hover:text-ink-900">
              Sign in
            </Link>
            <Link to="/register" className="btn-primary text-sm">
              Get started free
            </Link>
          </div>

          <button
            type="button"
            className="rounded-lg p-2 text-ink-600 md:hidden"
            onClick={() => setMenuOpen((o) => !o)}
            aria-label="Menu"
          >
            {menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </button>
        </div>

        {menuOpen && (
          <div className="border-t border-ink-100 bg-white px-5 py-4 md:hidden">
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <a
                  key={link.to}
                  href={link.to}
                  className="text-sm text-ink-700"
                  onClick={() => setMenuOpen(false)}
                >
                  {link.label}
                </a>
              ))}
              <Link to="/login" className="text-sm font-medium text-ink-700" onClick={() => setMenuOpen(false)}>
                Sign in
              </Link>
              <Link to="/register" className="btn-primary text-center text-sm" onClick={() => setMenuOpen(false)}>
                Get started free
              </Link>
            </div>
          </div>
        )}
      </header>

      <main className="flex-1">
        <Outlet />
      </main>

      <footer className="border-t border-ink-200/60 bg-white">
        <div className="mx-auto max-w-6xl px-5 py-12">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            <div className="lg:col-span-2">
              <Link to="/" className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-ink-900">
                  <MessageSquare className="h-4 w-4 text-white" />
                </div>
                <span className="text-sm font-semibold">SupportDesk</span>
              </Link>
              <p className="mt-3 max-w-sm text-sm leading-relaxed text-ink-500">
                AI customer support for small teams. Upload your knowledge base, embed a chat widget,
                and escalate to humans when it matters.
              </p>
            </div>

            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-ink-400">Product</p>
              <ul className="mt-3 space-y-2 text-sm text-ink-600">
                <li><a href="/#features" className="hover:text-ink-900">Features</a></li>
                <li><a href="/#pricing" className="hover:text-ink-900">Pricing</a></li>
                <li><Link to="/register" className="hover:text-ink-900">Create account</Link></li>
                <li><Link to="/agent" className="hover:text-ink-900">Agent portal</Link></li>
              </ul>
            </div>

            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-ink-400">Legal</p>
              <ul className="mt-3 space-y-2 text-sm text-ink-600">
                {legalLinks.map((l) => (
                  <li key={l.to}>
                    <Link to={l.to} className="hover:text-ink-900">{l.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
          </div>

          <div className="mt-10 flex flex-col gap-2 border-t border-ink-100 pt-8 text-xs text-ink-400 sm:flex-row sm:items-center sm:justify-between">
            <p>© {new Date().getFullYear()} SupportDesk. All rights reserved.</p>
            <p>
              Questions?{' '}
              <a href="mailto:support@supportdesk.app" className="text-ink-600 hover:text-ink-900">
                support@supportdesk.app
              </a>
            </p>
          </div>
        </div>
      </footer>
    </div>
  );
}
